---
title: "Significance Bands for Local Projetsions Using R"
author: 
  - name: "Itamar Caspi^[DISCLAIMER: The views expressed in this notebook are my own and do not represent the views of the Bank of Israel or any of its staff members.]"
...
output:
  html_document:
    ...
    toc_depth: 4
    toc_float: yes
abstract: This notebook employs the "Significance Bands for Local Projections" methodology, as detailed by [Inoue, Jordà, and Kuersteiner (2023)](https://arxiv.org/abs/2306.03073), henceforth referred to as IJK, to examine the impact of monetary policy shocks (Romer and Romer, 2023) on inflation. The importance of utilizing significance bands, as opposed to traditional confidence bands, is underscored by IJK, especially when evaluating the dynamic influence of a treatment on an outcome variable through local projections. I show how to implement these methods using the R programming language and provide a comprehensive, step-by-step guide aimed at assisting researchers who wish to apply these techniques to their own analyses.
---
...
Run local projections using the orthogonalized series. Specifically, estimate the given regression model:
...
The figure illustrates the impulse response of 100 times the logarithm of the GDP deflator following a Romer and Romer (2023) monetary olicy shock. The model includes four lagged variables for the GDP deflator, and the monetary policy shock. Dark and light green shaded areas indicate bands corresponding to one and two standard errors, respectively. For further details, refer to [Inoue, Jordà, and Kuersteiner (2023.)](https://arxiv.org/abs/2306.03073)
...
The following figure adds classical NW significance bands, shown in dashed blue and bootstrap significance bands, shown in dashed red.
...
plm_model <- plm(e_fy ~ factor(horizon):e_s, data = df_e_ys_long, index = c("horizon", "date"), model = "within")
```
